    ...
    def route(self, method, u, b, d):
        ...
        if p in ("/api/login", "/api/register") and method == "POST":
            ...
            if p == "/api/register":
                ...
                s = secrets.token_hex(8); d["users"][k] = {"nick": nick, "salt": s, "h": hpw(pw, s)}
        ...
        if p == "/api/post":
            ...
            if col == "chat":
                m = re.match(r"^/(ban|unban)\s+(\S+)$", text, re.I)
                if m:
                    if not adm: return 403, {"error": "forbidden"}
                    t = m.group(2).lower()
                    if t == ADMIN: return 200, {"ok": True}
                    if m.group(1).lower() == "ban":
                        if t not in d["banned"]: d["banned"].append(t)
                        d["tokens"] = {a: n for a, n in d["tokens"].items() if n.lower() != t}
                        msg = m.group(2) + " dostał bana."
                    else:
                        d["banned"] = [x for x in d["banned"] if x != t]; msg = m.group(2) + " został odbanowany."
                    d["chat"].append({"id": nid(), "nick": "System", "text": msg, "ts": int(time.time() * 1000)})
                    return 200, {"ok": True}
